...
class DailyCashFlowReport(models.AbstractModel):
    ...
    _description = "Cash Flow Report (By Period)"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # sort nhẹ cho đẹp
        payments = payments.sorted(key=lambda p: (p.payment_type != "bank", p.date_payment, p.id))
        receipts = receipts.sorted(key=lambda r: (r.payment_method != "bank", r.date, r.id))
