...
class ProjectWorkClaimProgress(models.Model):
    _name = "project.work.claim.progress"
    _description = "Claim/Settlement Progress Per Period"
    ...
    user_id = fields.Many2one(
        "res.users",
        related="assignment_id.claim_user_id",
    ...
    # tránh lỗi related company_id từ assignment (nếu assignment chưa có company_id)
    ...
    qty_cum = fields.Float(
        string="Lũy kế thanh/QT",
        digits="Product Unit of Measure",
        default=0.0,
    )
    qty_period = fields.Float(
        string="Khối lượng thanh/QT kỳ này",
        compute="_compute_qty_period",
        store=True,
        digits="Product Unit of Measure",
    )

    # Giá trị tính theo đơn giá hạng mục (fallback an toàn)
    ...
    value_cum = fields.Monetary(string="Lũy kế giá trị thanh/QT", compute="_compute_values", store=True)

    note = fields.Text(string="Ghi chú")

    _sql_constraints = [
        (
            "uniq_assignment_period_claim",
            "unique(assignment_id, period_no)",
            "Mỗi kỳ chỉ có 1 dòng thanh/quyết toán cho 1 phân công.",
        ),
    ]

    def _get_period_bounds(self, assignment, period_no):
        # dùng helper từ assignment extension
        if hasattr(assignment, "_claim_get_period_bounds"):
            return assignment._claim_get_period_bounds(assignment.date_start, period_no)

        # fallback cực đơn giản (không nên chạy tới đây)
    ...
    @api.depends("qty_cum", "period_no", "assignment_id", "assignment_id.claim_progress_ids.qty_cum")
    ...
    def _reported_cum_for(self, assignment, period_no):
        """
        Lấy lũy kế sản lượng báo cáo từ model gốc project.work.progress
        để chặn thanh/QT vượt sản lượng.
        """
    ...
    def write(self, vals):
        for rec in self:
            ...
            super(ProjectWorkClaimProgress, rec).write(safe_vals)
        ...
        for rec in self:
            ...
            if (rec.qty_cum or 0.0) < prev_cum:
                raise ValidationError(
                    f"Lũy kế thanh/QT kỳ {rec.period_no} phải >= lũy kế kỳ {rec.period_no - 1}."
                )
    ...
    def _check_not_exceed_reported(self):
        for rec in self:
            ...
            if (rec.qty_cum or 0.0) > (reported_cum or 0.0):
                raise ValidationError(
                    f"Không thể nhập lũy kế thanh/QT ({rec.qty_cum}) vượt lũy kế sản lượng báo cáo ({reported_cum}) "
