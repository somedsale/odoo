...
export class CommunityHubClientAction extends Component {
  ...
  setup() {
    ...
    // upload refs
    this.postFileInputRef = useRef("postFileInput");
    this.postImageInputRef = useRef("postImageInput");
    this.commentFileInputRef = useRef("commentFileInput");
    this.commentImageInputRef = useRef("commentImageInput");
    this._commentUploadPostId = null;
    ...
    this.state = useState({
      ...
      ui: {
        ...
        // composer extras (Teams-like)
        showPostEmoji: false,
        commentEmojiForPostId: null,
        emojiList: ["😀","😁","😂","🤣","😊","😍","👍","👏","🙏","🎉","🔥","😮","😢","😡","💯"],
        postAttachments: [],
        commentAttachments: {},
    ...
    this._onBusNotification = (ev) => {
      ...
      for (const n of list) {
        ...
        // invited/kicked -> toast + reload
        if (payload.type === "invited") {
          this.notification.add(
            `Bạn được mời vào community: ${payload.community_name || payload.community_id} (mời bởi ${payload.invited_by_name || "ai đó"})`,
            { type: "info" }
          );
          this.loadBootstrap();
          continue;
        ...
        if (payload.type === "kicked") {
          this.notification.add(`Bạn đã bị kick khỏi community #${payload.community_id}`, { type: "warning" });
          this.loadBootstrap();
          continue;
        }

        // notify_post/comment -> badge + toast (không cho chính mình)
        if (payload.type === "notify_post") {
          this._bumpBadge(payload.community_id);
          if (payload.actor_id && payload.actor_id !== this.state.user?.id) {
            this.notification.add(
              `${payload.actor_name || "Ai đó"} đăng bài mới trong ${payload.community_name || "community"} • #${payload.channel_name || ""}`,
              { type: "info" }
            );
          }
          continue;
        ...
        // cập nhật khi đang ở đúng community
  ...
  _resetSelection({ keepCommunities = true } = {}) {
    ...
    this.state.ui.showPostEmoji = false;
    this.state.ui.commentEmojiForPostId = null;
...
get isJoined() {
  ...
  // ===== Teams-like composer helpers =====
  togglePostEmoji() {
    this.state.ui.showPostEmoji = !this.state.ui.showPostEmoji;
  }

  onPostEmojiPick(ev) {
    const emoji = ev.currentTarget?.dataset?.emoji;
    if (!emoji) return;
    this.state.ui.postDraft = (this.state.ui.postDraft || "") + emoji;
  }

  toggleCommentEmoji(ev) {
    const postId = parseInt(ev.currentTarget?.dataset?.postId || "0", 10);
    if (!postId) return;
    this.state.ui.commentEmojiForPostId = (this.state.ui.commentEmojiForPostId === postId) ? null : postId;
  }

  onCommentEmojiPick(ev) {
    const postId = parseInt(ev.currentTarget?.dataset?.postId || "0", 10);
    const emoji = ev.currentTarget?.dataset?.emoji;
    if (!postId || !emoji) return;
    this.state.ui.commentDraft[postId] = (this.state.ui.commentDraft[postId] || "") + emoji;
  }
  ...
  openPostImagePicker() {
    this.postImageInputRef?.el?.click();
  }

  openCommentFilePicker(ev) {
    const postId = parseInt(ev.currentTarget?.dataset?.postId || "0", 10);
    if (!postId) return;
    this._commentUploadPostId = postId;
    this.commentFileInputRef?.el?.click();
  }
  openCommentImagePicker(ev) {
    const postId = parseInt(ev.currentTarget?.dataset?.postId || "0", 10);
    if (!postId) return;
    this._commentUploadPostId = postId;
    this.commentImageInputRef?.el?.click();
  }

  async onPostFilesChange(ev) {
    const files = Array.from(ev?.target?.files || []);
    ev.target.value = "";
    if (!files.length) return;
    try {
      const items = await this._uploadFiles(files);
      this.state.ui.postAttachments.push(...items);
    } catch (e) {
      this.notification.add(`Upload failed: ${e?.message || e}`, { type: "danger" });
    }
  }
  async onPostImagesChange(ev) {
    return this.onPostFilesChange(ev);
  }

  async onCommentFilesChange(ev) {
    const postId = this._commentUploadPostId;
    const files = Array.from(ev?.target?.files || []);
    ev.target.value = "";
    if (!postId || !files.length) return;
    try {
      const items = await this._uploadFiles(files);
      if (!this.state.ui.commentAttachments[postId]) this.state.ui.commentAttachments[postId] = [];
      this.state.ui.commentAttachments[postId].push(...items);
    } catch (e) {
      this.notification.add(`Upload failed: ${e?.message || e}`, { type: "danger" });
    } finally {
      this._commentUploadPostId = null;
  ...
  async onCommentImagesChange(ev) {
    return this.onCommentFilesChange(ev);
  }

  removePostAttachment(ev) {
    const attId = parseInt(ev.currentTarget?.dataset?.attId || "0", 10);
    if (!attId) return;
    this.state.ui.postAttachments = (this.state.ui.postAttachments || []).filter((a) => a.id !== attId);
  }

  removeCommentAttachment(ev) {
    const postId = parseInt(ev.currentTarget?.dataset?.postId || "0", 10);
    const attId = parseInt(ev.currentTarget?.dataset?.attId || "0", 10);
    if (!postId || !attId) return;
    const list = this.state.ui.commentAttachments[postId] || [];
    this.state.ui.commentAttachments[postId] = list.filter((a) => a.id !== attId);
  }
  ...
  async loadBootstrap() {
    try {
      ...
      this.state.selectedCommunity = data.selected || null;
        // ✅ nếu community đang selected nhưng chưa joined => không load kênh / bài viết
if (this.state.selectedCommunity && this.state.selectedCommunity.my_state !== "joined") {
  ...
  async refreshFeed() {
    ...
    try {
      ...
      this._clearBadge(this.state.selectedCommunity?.id);
  ...
  async joinCommunity(ev) {
    ...
    // dùng joinCommunity; api vẫn có alias join() để phòng assets cũ
  ...
  async submitPost() {
    ...
    const attachmentIds = (this.state.ui.postAttachments || []).map((a) => a.id);
  ...
  async submitComment(postId) {
    ...
    const attachmentIds = (this.state.ui.commentAttachments[postId] || []).map((a) => a.id);
...
// MUST match ir.actions.client tag
