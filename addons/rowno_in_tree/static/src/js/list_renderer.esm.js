  ...
  freezeColumnWidths() {
    ...
    if (!$(child_table.firstChild).hasClass("o_list_row_count_sheliya")) {
      const a = $(child_table).prepend(
        '<th class="o_list_row_number_header o_list_row_count_sheliya" style="text-align:center !important; max-width: 40px !important;">'
      );
      a.find("th.o_list_row_count_sheliya").html("#");
    }

    const footer = table.querySelector("tfoot tr");
    if (footer && !$(footer.firstChild).hasClass("o_list_row_count_sheliya")) {
      $(footer).prepend('<td class="o_list_row_count_sheliya"></td>');
    }
