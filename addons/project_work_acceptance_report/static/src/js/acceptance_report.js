...
export class AcceptanceReport extends Component {
  static template = "project_work_acceptance_report.AcceptanceReport";
  ...
  setup() {
    ...
    const storedProjectId = window.sessionStorage.getItem(
      "acceptance_project_id"
    );
    const storedSelectedPeriod = window.sessionStorage.getItem(
      "acceptance_selected_period"
    );
    const storedManagerMode = window.sessionStorage.getItem(
      "acceptance_manager_mode"
    );

    const projectId =
      contextProjectId || (storedProjectId ? Number(storedProjectId) : null);
    ...
    this.state = useState({
      ...
      availablePeriods: [],
      currentPeriodNo: 0,
      selectedPeriod: storedSelectedPeriod
        ? Number(storedSelectedPeriod)
        : null,
      rowsByPeriod: {},
      embeddedFormKey: 0,
    });
      ...
      setTimeout(() => {
        if (window.location.href === currentHref) {
          const fallbackActionXmlId = this.state.managerMode
            ? "project_work_acceptance_report.action_manager_acceptance_project_report"
            : "project_work_acceptance_report.action_my_acceptance_project_report";
  ...
  async syncWorkItemsFromSO(showNotice = true) {
    ...
    try {
      await this.orm.call("project.project", "action_sync_work_items_from_so", [
        [this.state.projectId],
      ]);
      if (showNotice) {
        this.notification.add("Đã đồng bộ hạng mục từ đơn bán.", {
          type: "success",
        });
      }
    } catch (error) {
      console.error(error);
      if (showNotice) {
        this.notification.add("Không thể đồng bộ hạng mục từ đơn bán.", {
          type: "warning",
        });
      }
    }
  ...
  async loadData() {
    ...
    if (!this.state.projectId) {
      this.notification.add("Không tìm thấy dự án để nhập nghiệm thu.", {
        type: "warning",
      });
    ...
    try {
      ...
      for (const [periodKey, rows] of Object.entries(
        result.rows_by_period || {}
      )) {
      ...
      this.state.projectAssignUserId =
        this.state.project.acceptance_user_id || null;
      ...
      if (!available.length) {
        ...
        this.state.selectedPeriod = available[available.length - 1];
      }
      ...
      if (this.state.selectedPeriod) {
        ...
        window.sessionStorage.removeItem("acceptance_selected_period");
      }

      this.state.embeddedFormKey += 1;
    } catch (error) {
      console.error(error);
      this.notification.add("Không tải được dữ liệu báo cáo nghiệm thu.", {
        type: "danger",
      });
      throw error;
    } finally {
  ...
  get isReadonlyPeriod() {
    ...
    return (
      Number(this.state.selectedPeriod) !== Number(this.state.currentPeriodNo)
    );
  }
...
onQtyInput(row, ev) {
  ...
  const prevQty = Number(row.prev_qty_cum || 0);
  const doneQty = Math.max(0, Number(row.done_qty || 0));

  // Không cho NT lũy kế vượt quá sản lượng đã thực hiện
  const maxCurrentQty = Math.max(0, doneQty - prevQty);
...
recomputeRowValues(row) {
  ...
  // Không cho lũy kế NT vượt quá khối lượng đã thực hiện
  let currentQtyCum = prevQty + currentQty;
  if (currentQtyCum > doneQty) {
    currentQtyCum = doneQty;
  }
  ...
  row.qty_remaining = Math.max(0, doneQty - currentQtyCum);
  ...
  const doneValueTax =
    row.done_value_tax !== undefined && row.done_value_tax !== null
      ? Number(row.done_value_tax || 0)
      : doneQty * priceUnitTax;
...
get totalRow() {
  ...
  async saveAll() {
    ...
    const dirtyRows = this.selectedRows
      ...
      .map((row) => ({
        acceptance_id: row.acceptance_id || false,
    ...
    try {
      ...
      this.notification.add("Đã lưu báo cáo nghiệm thu.", { type: "success" });
    } catch (error) {
      console.error(error);
  ...
  onProjectAssignUserChange(ev) {
    this.state.projectAssignUserId = ev.target.value
  ...
  async assignProjectUser() {
    ...
    try {
      const result = await this.orm.call(
        "project.project",
        "action_assign_project_acceptance_user",
        [[this.state.projectId], this.state.projectAssignUserId]
      );
      this.notification.add(
        result?.message || "Đã cập nhật người phụ trách nghiệm thu.",
        {
          type: "success",
        }
      );
      ...
      console.error(error);
      this.notification.add("Không thể cập nhật người phụ trách.", {
        type: "danger",
      });
      throw error;
    }
...
registry
  .category("actions")
  .add("project_work_acceptance_report.AcceptanceReport", AcceptanceReport);